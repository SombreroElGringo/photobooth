const photobooth = (() => {
   ...
   */
   const takePhoto = (id) => {
    ...
    if (WIDTH && HEIGHT) {
      ...
      displayElementsAs(id, ['take', 'mobile_camera'], 'none');
  ...
  const isOrientationPortrait = () => screen.availHeight > screen.availWidth;

  const isMobileOrTabletCheck = () => {
    let check = false;
  ...
  const adaptCameraConfig = (id, facingMode = 'user') => {
    let config = {
      video: {
        width: WIDTH,
        height: HEIGHT,
    ...
    if (isOrientationPortrait() && isMobileOrTabletCheck()) {
      config.video.aspectRatio = 1;
    }
    return initCamera(id, config);
  }
  ...
  const initCamera = (id, config) => {
    ...
    navigator.mediaDevices.getUserMedia(config).then((stream) => {
      ...
      displayElementsAs(id, ['select', 'take', 'mobile_camera', 'retake', 'save', 'output', 'spinner'], 'none');
      console.error(`An error occurred: ${error}`);
    });
  ...
  const initEventListeners = (id, overlays) => {
    ...
    const mobileCameraButton = getElementById(`${id}_mobile_camera`);
    ...
    mobileCameraButton.addEventListener('click', (event) => {
      stopMediaTracks();
      const facingMode = event.currentTarget.getAttribute('data-current-camera');
      event.currentTarget.innerText = facingMode === 'user' ? 'Front camera' : 'Back camera';

      const newFacingMode = facingMode === 'user' ? 'environment' : 'user';
      event.currentTarget.setAttribute('data-current-camera', newFacingMode);
      adaptCameraConfig(id, newFacingMode);
      event.preventDefault();
    }, false);
  ...
  return {
    initialize: (id, userId, overlays) => {
      if (userId === '0') {
        ...
        return displayElementsAs(id, ['select', 'take', 'mobile_camera', 'retake', 'save', 'camera', 'output', 'overlay', 'spinner'], 'none');
      }
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        ...
        displayElementsAs(id, ['retake', 'save', 'spinner'], 'none');
        if (!isMobileOrTabletCheck()) {
          displayElementsAs(id, ['mobile_camera'], 'none');
        }
        clearCanvas(id);
      } else {
        displayElementsAs(id, ['select', 'take', 'mobile_camera', 'retake', 'save', 'camera', 'output', 'overlay', 'spinner'], 'none');
